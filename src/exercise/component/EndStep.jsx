...
class End extends React.Component<Object> {
    ...
    render(): Element<*> {
        ...
        return <Wrapper modifier="small">
            <Box modifier="marginTopGiga">
                ...
                {(!result && !scorm.singleAttempt) && <Text modifier="block center marginGiga">{failDescriptionExtended}</Text>}
            </Box>
    ...
    renderDocumentHeader(name: string, file: string): Element<*>{
        let download = `./${file}`;
        return <a href={download} target="_blank" rel="noopener noreferrer" className="Button Button-secondary" style={{width: "120px"}}>Download PDF</a>;
    }
